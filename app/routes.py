...
from app.forms import RegistrationForm, LoginForm, CommentForm, SearchForm, AddFriendForm, SearchFriendForm
...
def register():
    ...
    if request.method == 'POST':
        ...
        country_data = next(
            (item for item in data if item["iso2"] == country_code), None)
        states = country_data["states"] if country_data else []
        state_data = next(
            (state for state in states if state["name"] == state_name), None)
        cities = state_data["cities"] if state_data else []

        form.state.choices = [(state["name"], state["name"])
                              for state in states]
        ...
        if form.validate_on_submit():
            ...
            if user:
                flash(
                    'An account with this email already exists. Please use a different email or login.', 'danger')
...
def dashboard():
    ...
    if request.method == 'POST':
        ...
        if from_country_code:
            country_data = next(
                (item for item in data if item["iso2"] == from_country_code), None)
            if country_data:
                form.from_state.choices = [
                    (state["name"], state["name"]) for state in country_data["states"]]
            if form.from_state.data:
                state_data = next(
                    (state for state in country_data["states"] if state["name"] == form.from_state.data), None)
                if state_data:
                    form.from_city.choices = [
                        (city["name"], city["name"]) for city in state_data["cities"]]

        if to_country_code:
            country_data = next(
                (item for item in data if item["iso2"] == to_country_code), None)
            if country_data:
                form.to_state.choices = [
                    (state["name"], state["name"]) for state in country_data["states"]]
            if form.to_state.data:
                state_data = next(
                    (state for state in country_data["states"] if state["name"] == form.to_state.data), None)
                if state_data:
                    form.to_city.choices = [
                        (city["name"], city["name"]) for city in state_data["cities"]]
        ...
        if form.validate_on_submit():
            ...
            post = Post.query.filter_by(
                from_location=from_location, to_location=to_location).first()
            if not post:
                post = Post(from_location=from_location,
                            to_location=to_location, user_id=current_user.id)
                db.session.add(post)
                db.session.commit()
...
def post(post_id):
    ...
    if reply_form.validate_on_submit():
        ...
        comment = Comment(content=reply_form.content.data, user_id=current_user.id,
                          post_id=post.id, parent_id=parent_comment.id)
        db.session.add(comment)
        db.session.commit()
        return redirect(url_for('main.post', post_id=post.id))

    return render_template('post.html', post=post, form=form, reply_form=reply_form, title="Post")
...
def friends():
    ...
    friends = User.query.join(Friend, (Friend.friend_id == User.id)).filter(
        Friend.user_id == current_user.id).all()
    search_results = []

    if search_form.validate_on_submit():
        search_query = search_form.search.data
        search_results = User.query.filter(
            User.email.like(f"%{search_query}%")).all()

    return render_template('friends.html', title='Friends', friends=friends, search_form=search_form, add_friend_form=add_friend_form, search_results=search_results)
